...
export function ScrollVideoHero() {
  ...
  const locationLabel = activeLoc.shortName;
  const addressLabel = activeLoc.street;
  ...
  // Decide whether scroll-scrubbing is enabled (disabled only for prefers-reduced-motion)
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (v && v.readyState >= 2 && !v.seeking) {
        ...
        if (Math.abs(delta) > 0.03) {
          const nextTime = current + delta * 0.15;
          if (
            "fastSeek" in v &&
            typeof (v as { fastSeek?: (t: number) => void }).fastSeek === "function"
          ) {
            (v as { fastSeek: (t: number) => void }).fastSeek(nextTime);
          } else {
  ...
  // Fallback playback: play through if scrubbing is disabled
  useEffect(() => {
    const video = videoRef.current;
  ...
  return (
    <section
      ref={wrapperRef}
      aria-label="Bro 502 Barber introduction"
    ...
    >
      <div className="sticky top-0 flex h-[100svh] items-center justify-center overflow-hidden">
        ...
        >
          ...
          >
            {locationLabel}, Seattle · Est. 2014
          </motion.p>
          <motion.h1
